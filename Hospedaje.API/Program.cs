...
{
    options.SwaggerDoc("v1", new OpenApiInfo { Title = "Microservicio Hospedaje API", Version = "v1" });
...
builder.Services.AddDbContext<HospedajeDbContext>(options =>
...
builder.Services.AddScoped<ICargoEstadiaRepository, CargoEstadiaRepository>();
builder.Services.AddScoped<IEstadiaDataService, EstadiaDataService>();
...
app.Logger.LogInformation("Iniciando {Service} en ambiente {Environment}", "Microservicio.Hospedaje", app.Environment.EnvironmentName);
app.Logger.LogInformation("Swagger habilitado en /swagger y /swagger/v1/swagger.json");
app.Logger.LogInformation("Cadena de conexion DefaultConnection cargada para {Service}", "Microservicio.Hospedaje");
app.UseSwagger();
app.UseSwaggerUI(options => options.SwaggerEndpoint("/swagger/v1/swagger.json", "Microservicio Hospedaje API v1"));
app.UseMiddleware<Hospedaje.API.Middleware.ExceptionHandlingMiddleware>();
...
app.MapGet("/health", () => Results.Ok(new { status = "ok", service = "Microservicio.Hospedaje" }));
app.MapGet("/health/db", async (HospedajeDbContext dbContext, ILoggerFactory loggerFactory) =>
{
    var logger = loggerFactory.CreateLogger("HealthDb");
    try
    {
        logger.LogInformation("Validando conexion a base de datos de Hospedaje");
        var canConnect = await dbContext.Database.CanConnectAsync();
        return canConnect
            ? Results.Ok(new { status = "ok", database = "connected", service = "Microservicio.Hospedaje" })
            : Results.Problem("No fue posible conectar a la base de datos de Hospedaje.", statusCode: StatusCodes.Status503ServiceUnavailable);
    }
    catch (Exception ex)
    {
        logger.LogError(ex, "Error validando conexion a base de datos de Hospedaje");
        return Results.Problem(ex.Message, statusCode: StatusCodes.Status503ServiceUnavailable);
    }
});
app.MapControllers();
app.Logger.LogInformation("{Service} listo para recibir solicitudes", "Microservicio.Hospedaje");
app.Run();
